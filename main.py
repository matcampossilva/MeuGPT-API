...
async def whatsapp_webhook(request: Request):
    ...
    try: # Bloco try principal
        ...
        # --- FLUXO: DEFINIR LIMITES (Interpretação e Confirmação) --- 
        if estado.get("ultimo_fluxo") == "aguardando_definicao_limites":
            ...
            limites_pendentes = [] # Lista para armazenar limites interpretados para confirmação
            limites_erro_parse = [] # Erros durante a interpretação inicial

            for linha in linhas:
                partes = linha.split(":")
                if len(partes) == 2:
                    # Capitaliza categorias conhecidas, mantém outros itens como estão (ex: iFood)
                    ...
                    try:
                        ...
                        limite_interpretado = {
                            "item_ou_categoria": item_ou_categoria,
                            "valor": valor
                        }
            ...
            if limites_pendentes:
                ...
                estado["ultimo_fluxo"] = "aguardando_confirmacao_limites" # NOVO ESTADO
                estado_modificado_fluxo = True
            else:
                # Se não entendeu NENHUM limite válido
        ...
        # --- FLUXO: CONFIRMAÇÃO DE LIMITES (Registro ou Edição) --- 
            ...
            if "sim" in resposta_usuario_lower or "yes" in resposta_usuario_lower or "confirmo" in resposta_usuario_lower:
                ...
                for limite in limites_pendentes:
                    try:
                        # Chama a função para salvar o limite individualmente
                ...
                # Limpa os limites pendentes e o fluxo do estado
                ...
                # Mantém o estado aguardando_confirmacao_limites
        ...
        elif any(term in msg_lower for term in ["definir limites", "limites por categoria", "colocar limites", "estabelecer limites", "limite de gasto"]) and estado.get("ultimo_fluxo") not in ["aguardando_definicao_limites", "aguardando_confirmacao_limites"]:
             logging.info(f"{from_number} pediu para definir limites.")
             msg_instrucao_limites = (
                 "Entendido! Para definir seus limites, envie a categoria (ex: Lazer) ou um item específico (ex: iFood) e o valor mensal, um por linha. Exemplo:\n"
                 "Lazer: 500\n"
                 "Alimentação: 1500\n"
                 "iFood: 400\n"
                 "Transporte: 300"
        ...
        # --- FLUXO: CONFIRMAÇÃO DE GASTOS FIXOS (Registro ou Edição) --- 
            ...
            if "sim" in resposta_usuario_lower or "yes" in resposta_usuario_lower or "confirmo" in resposta_usuario_lower:
                ...
                # Se não há pendentes, pergunta sobre lembretes (se algo foi salvo)
                    ...
                    estado["ultimo_fluxo"] = "aguardando_decisao_lembretes_fixos" # ESTADO RENOMEADO
                else:
        ...
        # --- FLUXO: DECISÃO SOBRE LEMBRETES (Gastos Fixos) --- 
        elif estado.get("ultimo_fluxo") == "aguardando_decisao_lembretes_fixos": # ESTADO RENOMEADO
            resposta_usuario_lower = msg_lower
        ...
        # --- FLUXO: CONFIRMAÇÃO DE GASTO DIÁRIO --- 
            ...
            if not gasto_pendente:
                ...
                if categoria_informada_cap in CATEGORIAS_VALIDAS:
                    ...
                    # AQUI entraria a lógica de edição (Futuro)
